...
class ExampleServer {
 public:
  ...
  virtual int do_rpc_service(Echo::Request* req, Echo::Response* resp,
                             IOVector*, IStream*) {
    photon::std::this_thread::migrate();
    rocksdb::Status s;
    std::string val;
    rocksdb::DB* db = db_sharding[std::stoi(std::string(req->key)) % m_db_num];
    if (req->write) {
...
class MultiDBExampleServer : public ExampleServer {
 public:
  ...
  int do_rpc_service(Echo::Request* req, Echo::Response* resp, IOVector*,
    ...
    rocksdb::Status s;
    std::string val;
    size_t index = std::stoi(std::string(req->key)) % m_db_num;

    // TODO: modify photon
    // photon::std::this_thread::migrate(index);
...
void test_single_server() {
  photon::std::work_pool_init(FLAGS_vcpu_num, photon::INIT_EVENT_IOURING, 0);
